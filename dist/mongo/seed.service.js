...
let SeedService = class SeedService {
    ...
    async seedPolicyPages() {
        ...
        for (const [key, value] of Object.entries(pages)) {
            const existing = await this.mongo.findOne('business_settings', { key });
            if (existing) {
    ...
    async seedConversations() {
        ...
        const restaurants = await this.mongo.findMany('restaurants', {}, { limit: 3 });
        const dms = await this.mongo.findMany('delivery_men', {}, { limit: 3 });
        if (users.length === 0)
        ...
        for (const u of users) {
            const existing = await this.mongo.findOne('conversations', { user_id: u.mysql_id });
            if (existing)
                continue;
    ...
    async seedSubscriptionOrders() {
        const users = await this.mongo.findMany('users', {}, { limit: 1 });
        const subs = await this.mongo.findMany('subscriptions', {}, { limit: 5 });
