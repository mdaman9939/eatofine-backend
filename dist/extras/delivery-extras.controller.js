...
let DeliveryExtrasController = class DeliveryExtrasController {
    ...
    async earningReport(req) {
        ...
        const empty = {
            ...
            trends: { categories: [], earning_series: [], expense_series: [] },
            transactions: { total_size: 0, limit: 0, offset: 0, data: [] },
        };
        if (!this.useMongo())
            return empty;
        const actorId = Number(req.actor.id);
        const orders = await this.mongo.findMany('orders', { mysql_delivery_man_id: actorId, order_status: 'delivered' }, { sort: { mysql_id: -1 }, limit: 500 });
        ...
        for (const o of orders) {
            ...
            totalDelivery += dc;
            totalTips += tip;
            const tsRaw = (o.delivered ?? o.created_at);
            ...
            if (idx >= 0 && idx < 7)
                dayBuckets[idx] += dc + tip;
        }
        const claims = await this.mongo.findMany('dm_reward_claims', { dm_id: actorId, status: 'approved' });
